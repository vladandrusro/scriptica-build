...
18. **Persona „Utilizator Intern PMB” + client Primăria Municipiului București** (2026-08-18) — `pmb_intern` in shell.js sees only the HQ client `cli_pmb` (`tenantPersona` binds account ↔ persona; the account never leaks into other personas via tenant preview). New client category `ct_institutie_publica` (`archiveRouting: 'nomenclator'` — archive folders are nomenclator indicatives X.a.1 grouped by vertical, documents route by type, container = the direcție via `flowItem.archiveContainer`). Six verticals in state-administration language: Resurse Umane, Achiziții Publice, Investiții și Lucrări, Solicitări Externe, Solicitări Interne (ad-hoc template always first), Asistentul AI Scriptica. Per-vertical `externalParty` (Petent, Operator economic…) with account fallback. PMB employees replace `MOCK.employees` while the persona is active.
19. **Asistentul AI Scriptica** (2026-08-18) — conversations with the local LLM are „cereri de lămuriri”: a one-step flow in the `assistant: true` vertical. `js/asistent-ai.js` turns the Mesagerie panel of situatie-detaliu into the chat (collapsible reasoning, answer with sources, context shift, prompt chips, stop/copy/feedback), driven by a deterministic local search over the flow items/documents/people the persona may access. `shell.js` injects the „Asistent AI” button into every Mesagerie header when the tenant has an assistant vertical; `scripticaCreateAiRequest()` creates the request and opens it.
20. **Clean-URL fix** (2026-08-18) — Cloudflare Pages serves `/acasa` without `.html`; filename checks in dashboard-widgets.js, dashboard.js and shell.js (active nav) now normalise the extension. Before this, configured dashboards never mounted on the live site.
21. **Organigrama PMB** (2026-08-20, obs. document 3 AUG 2026 — „similar cu eSCIM dar mai bine”) — new page `organigrama.html` + `js/organigrama.js` + `css/organigrama.css` (prefix `.org-`), nav item „Organigramă” injected by shell.js right after Acasă for tenants whose client type has `archiveRouting: 'nomenclator'`. Data: `MOCK.pmb.organigrama` (mock-data.js) transcribes the official PMB organigram (conducere, 9 direcții generale with children, 18 structuri în subordinea directă, post counts). Pannable/zoomable canvas (drag + wheel, Ctrl/⌘+wheel zoom, fit), structure search with highlight/dim. Structures whose `directieCode` matches a nomenclator direcție are „live”: the card (and its three icons) opens a right-side drawer with tabs **Dosare** (nomenclator folders grouped by serviciu, retention, doc counts, deep link `arhiva.html?folder=<id>` — arhiva.js also accepts `?dir=<dirKey>`), **Fluxuri** (flow items with `archiveContainer` = direcție name, status pills, deadlines from template steps), **Notificări** (derived: overdue steps, spre_aprobare, unread messages, closed-year archive notice). The Primar General card opens the whole-institution synthesis. Other personas: no nav item and a guard message on direct access.
22. **Nomenclatorul arhivistic de stat + „Arhivare la finalizare”** (2026-08-20, obs. document 3 AUG 2026) — `ct_institutie_publica.archiveTree` follows the state structure: every dosar carries `code` (X.a.1), `directie`/`directieCode`, `serviciu`/`serviciuCode`, `retention`; the PMB archive navigates Direcție → Serviciu → An → dosar (arhiva.js reuses the generic levels; directions sorted by indicativ, empty nomenclator structures visible, years < 2026 locked as „an închis”, retention strip on dosar selection). DMRU (X) includes the two dosare from the state document (X.a.1 inventare/PV predare-primire, X.a.2 R.O.F./stat/scheme). The archive destination is chosen **per anexă**: `template.anexaArchiveFolders[anexaId]` („Dosar la finalizarea fluxului” under each attached anexă in the Fluxuri V2 step inspector; options are the vertical's nomenclator dosare via `scripticaNomenclatorFoldersForVertical`; the same anexă may map to different dosare in different flows — e.g. PV concurs → X.b.1 in recrutare, X.b.2 in promovare; `template.archiveFolderId` survives only as a legacy fallback at completion time). On final-step completion situatie-detaliu.js generates one document per mapped anexă into its dosar, named `[indicativ]_[anexa]_nr-[nr]_[data].pdf`, with `doc.archiveFolderId` taking routing priority in arhiva.js. All seeded PMB document filenames were renamed to the same convention. The PMB templates with anexe seed `anexaArchiveFolders` maps. Inside a nomenclator flow's Documente section, the tabs ARE the archive dosare (documents.js builds folder pseudo-categories from the relevant indicatives; reclassification also rewrites `doc.archiveFolderId` so Arhivă follows); the state-document example X.a.1 is fed by the seeded flow `ft_pmb_ru_predare_arhiva` („Predare-primire dosare la depozitul de arhivă”, item `fi_pmb_ru_06`, new RU category „Predare la arhivă” + inventar anexă). ⚠️ Stale-state note: a stale persisted `scriptica.clientTypes` record for `ct_institutie_publica` shadows the new tree (folders then lack directie/serviciu and group under „Necategorisit”) — the documented remedy is clearing localStorage.
...
7. **Ad-hoc internal requests vs. AI requests** (Vlad, 2026-08-18) — should ad-hoc internal requests always go through the AI assistant, or stay a separate free flow? Both are kept for now (ad-hoc template first in Solicitări Interne + Asistentul AI vertical) so the PMB team can test; the wider design of ad-hoc flows for internal users is a pending discussion.
