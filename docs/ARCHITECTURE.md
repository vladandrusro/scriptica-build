...
Client types with `archiveRouting: 'nomenclator'` (public institutions) get no per-template flow folders: their `archiveTree` holds the state nomenclator arhivistic dictated by the organigramă. Each folder carries `code` (indicativ, e.g. `X.a.1`), `directie`/`directieCode`, `serviciu`/`serviciuCode`, `retention` (termen de păstrare) and keeps `group` = vertical name (the link flows use). arhiva.js reuses its generic four tree levels as Direcție → Serviciu → An → dosar for this mode (containers sorted by nomenclator order, empty structures still shown, years before the pinned demo year rendered locked/„închis”, retention shown when a dosar is selected). Documents route by document type, and `doc.archiveFolderId` (set when a flow archives its anexe on completion) takes priority over type matching. A flow template maps each attached anexă to its own dosar in `anexaArchiveFolders[anexaId]` — the „Dosar la finalizarea fluxului” select under every anexă in the Fluxuri V2 step inspector (options via `scripticaNomenclatorFoldersForVertical`, lookups via `scripticaArchiveFolderById`; the legacy `template.archiveFolderId` is honored only as completion-time fallback); when the last step of such a flow completes, situatie-detaliu.js generates one `source: 'generat'` document per mapped anexă, named `[indicativ]_[anexa-slug]_nr-[nr]_[data].pdf`, stored in the flow item's `documents` (persisted) and merged into `MOCK.documents` at load. Verticals may carry `externalParty {singular, plural}` — `scripticaEffectiveExternalParty(client, verticalId)` prefers it over the account label. Verticals with `assistant: true` are served by js/asistent-ai.js.
